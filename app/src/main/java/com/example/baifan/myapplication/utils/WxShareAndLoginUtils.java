...
public class WxShareAndLoginUtils {
    ...
    public static IWXAPI getWXAPI(){
        if (iwxapi == null){
            ...
            iwxapi = WXAPIFactory.createWXAPI(App.getContext(), Config.APP_ID, true);
            //将应用的appid注册到微信
    ...
    public static void WxBitmapShare(Bitmap bitmap, int judge){
        ...
        bitmap.recycle();
        message.thumbData = FileUtil.bmpToByteArray(thunmpBmp,true);
    ...
    public static void WxUrlShare(String url,String title,String description,Bitmap bitmap,int judge){
        ...
        Bitmap thunmpBmp = Bitmap.createScaledBitmap(bitmap,50,50,true);
        bitmap.recycle();
        wxMediaMessage.thumbData = FileUtil.bmpToByteArray(thunmpBmp,true);
    ...
    private static boolean judgeCanGo(){
        ...
        if (!iwxapi.isWXAppInstalled()) {
            Toast.makeText(App.getContext(), "请先安装微信应用", Toast.LENGTH_SHORT).show();
            return false;
        } else if (!iwxapi.isWXAppSupportAPI()) {
            Toast.makeText(App.getContext(), "请先更新微信应用", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
